...
export default function Page() {
  ...
  const [mode, setMode] = useState<Mode>("enkelt");
  ...
  return (
    <div className="relative z-10 flex-1 flex flex-col">
        ...
        <div className="max-w-[640px] mx-auto">
          ...
          <header className="reveal pt-8 pb-6">
            ...
            <p className="text-ink-soft mt-3 max-w-[44ch]">
              Velg abonnement og rabatter og se månedspris og besparelse mot full pris.
            </p>
          </header>

          {/* Mode tabs */}
          <div className="reveal inline-flex rounded-full border border-line bg-card p-1 mb-6">
            {(["enkelt", "familie"] as Mode[]).map((m) => (
              <button
                key={m}
                type="button"
                onClick={() => setMode(m)}
      ...
      {/* Sticky total bar (Enkelt only; Familie shows its own total inline) */}
      {mode === "enkelt" && (
